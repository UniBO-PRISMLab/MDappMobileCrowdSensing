...
class VerifierCampaignDataController extends StatefulWidget {
  final String goTo;
  const VerifierCampaignDataController({Key? key, required this.goTo})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(children: [
        SizedBox(
            height: DeviceDimension.deviceHeight(context) / 4,
            child:
            Column(children: [
              ...
              //Text("uploaded $fileCount files\nchecked $fileChecked of $fileCount", style: CustomTextStyle.spaceMonoBold(context),),
            ])
        ),
        FutureBuilder(
            future: _formatData(),
              ...
              switch (snapshot.connectionState) {
                case ConnectionState.none:
                  return const Expanded(
                      flex: 1,
                      child:
                          Center(child: Text('Sorry something goes wrong...')));
            ...
            itemBuilder: (context, index) {
              ...
              if (status == 'false') {
                return GestureDetector(
                  onTap: () {
                    if (SessionModel().getAccountAddress().toLowerCase() !=
                        uploader.toLowerCase()) {
                      Navigator.pushNamed(context, widget.goTo, arguments: {
                        "name": name,
                        "ipfsHash": ipfsHash,
                        "uploader": uploader,
